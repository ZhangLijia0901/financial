...
public class JdkArrayList<E> extends AbstractList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 *
	 * @param initialCapacity 初始容量大小<br>
	...
	/**
	 * 构造一个指定集合元素的列表, 按集合迭代器返回的顺序, 存储到新的集合列表中<br>
	...
	public JdkArrayList(Collection<? extends E> c) {
		...
		if ((size = elementData.length) != 0) {
			...
			if (elementData.getClass() != Object[].class)// 判断c.toArray 获取的不是Objcet[]
				elementData = Arrays.copyOf(elementData, size, Object[].class);
		} else {
			// replace with empty array.
			this.elementData = EMPTY_ELEMENTDATA;// 指定集合不存在数据
		}
	}
	...
	private void ensureCapacityInternal(int minCapacity) {
		if (elementData == DEFAULTCAPACITY_EMPTY_ELEMENTDATA) {// 判断表元素是空构造未初始数组实例
			minCapacity = Math.max(DEFAULT_CAPACITY, minCapacity);// 取默认初始容量与至少容量中较大的一个
		}

		ensureExplicitCapacity(minCapacity);// 检查明确容量
	}
...
// overflow-conscious code
		if (minCapacity - elementData.length > 0)// 判断至少容量>元素数据数组大小
			grow(minCapacity);// 扩容
	}
	 ...
	 *
	 * @param minCapacity 最小容量<br>
	 *                    the desired minimum capacity
	 */
	private void grow(int minCapacity) {
// overflow-conscious code
		int oldCapacity = elementData.length;// 原容量大小
		int newCapacity = oldCapacity + (oldCapacity >> 1);// 新容量 = 原容量 + 原容量/2
		if (newCapacity - minCapacity < 0)// 新容量<最小容量(主要针对原容量大小为1的情况)
			newCapacity = minCapacity;// 使用最小容量为新容量
		if (newCapacity - MAX_ARRAY_SIZE > 0)// 新容量>最大数组大小
			newCapacity = hugeCapacity(minCapacity);// 巨大容量获取新容量大小
// minCapacity is usually close to size, so this is a win:
		elementData = Arrays.copyOf(elementData, newCapacity);// copy一个新的数组
	}
	...
	private static int hugeCapacity(int minCapacity) {
		...
		return (minCapacity > MAX_ARRAY_SIZE) ? Integer.MAX_VALUE : MAX_ARRAY_SIZE;// 判断至少容量>最大数组大小则使用int最大值否则使用最大数组大小
	}
...
// Positional Access Operations
	...
	public void add(int index, E element) {
		...
		// 检查容量+
		ensureCapacityInternal(size + 1); // Increments modCount!!
		System.arraycopy(elementData, index, elementData, index + 1, size - index);// 将下标开始的元素向后移动一位
		elementData[index] = element;// 将元素添加到指定下标
		size++;
	}
	...
	public E remove(int index) {
		...
		modCount++;// 记录修改次数
		E oldValue = elementData(index);// 根据下标获取元素数据

		int numMoved = size - index - 1;// 删除元素位置之后的元素个数(总数量-(删除下标+1))
		if (numMoved > 0) // 判断删除元素之后存在元素
			System.arraycopy(elementData, index + 1, elementData, index, numMoved);// 将删除下标以后的元素向前移动一位
		// 将最后一个元素置空
	...
	/**
	 * 根据下标删除元素 跳过下标越界检查 <br>
	 * Private remove method that skips bounds checking and does not return the
	 * value removed.
	 */
	private void fastRemove(int index) {
		modCount++;// 记录修改次数
		int numMoved = size - index - 1;// 获取删除元素之后的元素个数
		if (numMoved > 0)// 判断删除元素之后存在元素
			System.arraycopy(elementData, index + 1, elementData, index, numMoved);// 将之后元素向前移动一位
		// 将最后一个元素置空
		elementData[--size] = null; // clear to let GC do its work
	}
